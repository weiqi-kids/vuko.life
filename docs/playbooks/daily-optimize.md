...
#### 判讀昨日賭注：「某頁 GSC 逐筆多天零變化」不等於資料抓取壞掉

GSC 的每日快照是**滑動 7 天視窗**（右緣＝當日 −3 天延遲）。當某頁的曝光**叢集在少數幾天**，
只要視窗左右緣都還罩著那幾天，該頁的逐筆 query 加總就會**連續多天逐位元完全相同**——這是滑動視窗
的正常數學，不是 API 快取／抓取故障。要區分「凍結是 bug」還是「叢集在視窗重疊區」，用兩個判準：

1. **同期 `gsc.totals.impressions` 有沒有在變？** 若總曝光每天仍在變（例如首頁的貢獻在動），代表
   API 回的是新鮮、逐日不同的資料，那「某單頁凍結」必然是叢集效應，**不是抓取問題**。
2. **回推叢集落在哪天**：把該頁 Σimp 對「視窗右緣日」畫出來——Σ 從 0 跳升的那天，右緣日就是曝光
   發生日；Σ 之後持平，代表左緣還沒滑過最早那筆。等左緣滑過，Σ 會自動下降到 0。可據此**預測**它哪天
   解凍，用明天的快照證實／證偽。

**只有當「總曝光也整包不變、且連續多日」時**才懷疑抓取／collect 故障，才值得標 🟡 請人工到 GSC UI 複核。
單頁凍結＋總曝光在動＝正常，run-log 註明「視窗重疊叢集、預計 X 日解凍」即可，**不要標 🟡、不要每天重改同一頁**。

> 實例（2026-07-20 複核）：`40-hz-gamma-binaural-beats.html` 8 筆 query 於 07-16~07-20 連續 5 天逐位元
> 相同，一度被疑為抓取故障。實查為**真實叢集**：該頁只在 07-11（6 次）、07-12（7 次）有曝光，其餘每天 0；
> 7 天視窗罩住這兩天故 Σ 固定 13，同期總曝光仍在 52→43 波動。預測 07-22 檔（視窗 07-12..07-19，掉 07-11）
> Σ 降至 7、07-23 檔（掉 07-12）歸 0。**結論：非 bug，勿標 🟡、勿再動該頁。**
